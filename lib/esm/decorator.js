 ...
 * ```
 */
export function check(target, propertyKey, parameterIndex) {
 ...
 * ```
 */
export function checked(...runtypes) {
        ...
        descriptor.value = function (...args) {
            runtypes.forEach((type, typeIndex) => {
                ...
                if (!validated.success) {
                    throw new ValidationError(`${methodId}, argument #${parameterIndex}: ${validated.message}`, validated.key);
                }
            });
